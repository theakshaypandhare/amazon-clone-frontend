...
export const Product = () => {
  const location = useLocation();
    const { id } = location.state || "mbl001";
        ...
        <Grid item md={5} sm={6} xs={12}>
          <Box>
            <Typography variant="h5">
              {item.brand + " " + item.modelName} 5G ({item.color} , {item.ram}{" "}
              RAM, {item.storage} Storage) |{item.displayType} Display |{" "}
              {item.processor} 5G Processor | {item.cameraPixel} Camera |{" "}
              {item.specialty}
            </Typography>
